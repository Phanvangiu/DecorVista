...
// activeBlog
const acticeBlog = async (id) => {
  const response = await axiosAdmin.get("blog/activeBlog?id=" + id);
  return response.data;
};
export function ActiveBlog(id) {
  return useQuery({
    queryKey: ["activeBlog", id],
    queryFn: () => acticeBlog(id), // Pass ownerBlogId to the function
  });
}
