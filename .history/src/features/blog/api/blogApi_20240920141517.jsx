...
// activeBlog
const acticeBlog = async (id, decision) => {
  const response = await axiosAdmin.get(
    "blog/activeBlog?id=" + id + "&yes=" + decision
  );
  return response.data;
};
export function ActiveBlog(id, decision) {
  return useQuery({
    queryKey: ["activeBlog", id, decision],
    queryFn: () => acticeBlog(id, decision), // Pass ownerBlogId to the function
  });
}
